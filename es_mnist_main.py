...
def train_model_es(
    epoch: int, model, train_loader, es_estimator, device, criterion, args, alpha: float
) -> tuple[float, float]:
    """
            ...
            for i in range(es_estimator.num_pert):
                ...
                for name, param in model.named_parameters():
                    if not param.requires_grad:
                        continue
                    noise = torch.randn(
                        param.shape, device=param.device, dtype=param.dtype, generator=rng
                    )
...
class ESSetting(FrozenSetting):
    ...
    alpha: float = Field(default=0.002, description="Learning rate (alpha) for ES update rule")
